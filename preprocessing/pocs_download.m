function [kspFull,im] = pocs_download( ksp, iter, watchProgress )
...
%               and create a zero-padded array with the funew8ll size, first.
...
%
Ntrans = floor( (szSym-1)/4 );      % width of the transition zone
...
kspFull = im;
if nargout == 1
    im = double.empty([sz 0]);     % im exists, but no memory required
end
...
filter = tukeywin(sz(1),0.1)*tukeywin(sz(2),0.05).';
kspFull = bsxfun(@times, reshape(filter, [1 sz(1:2)]), kspFull);
if nargout > 1
    im = kspFull;
    for ii = 2:d+1
        im = ifftshift(ifft(ifftshift(im,ii),[],ii),ii);
    end
end
end     % of pocs()
% =========================================================================
