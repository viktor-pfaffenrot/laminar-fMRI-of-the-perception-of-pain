...
if strcmp(experiment,'WM_localizer')
    experiment_ID = 'wmpractice';
elseif strcmp(experiment,'pain_calib')
    experiment_ID = 'calibration';
elseif strcmp(experiment,'post_calib')
    experiment_ID = 'postcalibration';
else
    experiment_ID = 'maintask';
end

experiment_path = [experiment_path '/func/' experiment];
...
inputs = cell(0, 1);
spm('defaults', 'FMRI');
spm_jobman('run', batch_to_execute(stats_path,scans,timing_info_file.loginfo.(experiment_ID),experiment_ID), inputs{:});

end

function matlabbatch = batch_to_execute(stats_path,scans,timing_info,experiment_ID)
...
if ~strcmp(experiment_ID,'wmpractice')
    task = 'painstim';
else
    task = 'task';
end
...
for ii = 1:length(fieldnames(scans))
    ...
    motion_file = [stats_path(1:pos) 'run' num2str(ii) '/func/mag_POCS_r' num2str(ii) '_MoCorr.txt'];
    if ~isfile(motion_file)
        movefile([motion_file(1:end-4) '.params'],[motion_file(1:end-4) '.txt']);
%         system(['mv ' '"' motion_file(1:end-4) '.params' '"' ' ' '"' [motion_file(1:end-4) '.txt'] '"']);
    end
